...
// keys: 未命中的键列表
// 返回: 键值映射, 找到的键列表, 错误信息
type BatchFallbackFunc func(ctx context.Context, keys []string) (map[string]interface{}, []string, error)
...
type Cacher interface {
	...
	// keys: 要获取的键列表
	// dstMap: 目标map的指针，用于接收结果，类型为map[string]*T
	// fallback: 批量回退函数，处理未命中的键
	// opts: 缓存选项，可以为nil使用默认选项
	// 返回: 找到的键列表, 错误信息
	MGet(ctx context.Context, keys []string, dstMap interface{}, fallback BatchFallbackFunc, opts *CacheOptions) ([]string, error)
